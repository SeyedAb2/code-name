...
["02","02-install.html",1,18,110,"نصب روی ویندوز، لینوکس و WSL2","Engine در برابر Desktop، راه‌اندازی WSL2 و رفع خطاهای رایج نصب.","Installing on Windows, Linux and WSL2","Engine vs Desktop, WSL2 setup, and the usual installation failures.","install wsl2 desktop engine systemd نصب"],
["03","03-run.html",1,18,105,"اجرا، لاگ، exec و پورت","چرخهٔ کامل کار با یک کانتینر زنده: run، ps، logs، exec، stop، rm.","Run, logs, exec and ports","The full loop with a live container: run, ps, logs, exec, stop, rm.","run ps logs exec stop rm port publish detach tty"],
["04","04-dockerfile.html",1,18,110,"نوشتن Dockerfile؛ لایه‌ها و کش","هر دستور یک لایه است. ترتیب دستورها یعنی تفاوت بیلد ۲ ثانیه‌ای و ۲ دقیقه‌ای.","Writing a Dockerfile; layers and cache","Every instruction is a layer. Order is the difference between a 2-second and a 2-minute build.","dockerfile from run copy cmd entrypoint layer cache dockerignore"],
["05","05-multistage.html",1,18,120,"multi-stage build و کوچک‌کردن ایمیج","جدا کردن محیط build از runtime و سنجش اندازهٔ واقعی artifact.","Multi-stage builds and slim images","Separating build from runtime and measuring the real artifact.","multi-stage builder alpine distroless slim size"],
["06","06-data.html",1,18,120,"داده: volume، bind mount و پشتیبان‌گیری","writable layer، named volume، bind mount و backup را با آزمایش واقعی و یک لَب PostgreSQL از هم جدا کن.","Data: volumes, bind mounts, and backups","Compare the writable layer, named volumes, bind mounts, and backups through hands-on experiments and a PostgreSQL lab.","volume bind mount writable layer backup restore postgres data persistence"],
["07","07-network.html",1,18,120,"شبکه: bridge، DNS داخلی و publish","localhost، network مشترک، DNS داخلی و تفاوت traffic داخل Docker با publish روی host را با دو container عیب‌یابی کن.","Networking: bridge networks, internal DNS, and port publishing","Diagnose localhost, shared networks, internal DNS, and the difference between Docker traffic and host publishing with two containers.","network bridge dns localhost publish port container name connect disconnect isolation"],
["08","08-compose.html",1,18,140,"Docker Compose: چند سرویس، یک مدل","با ساخت تدریجی compose.yaml، ارتباط web و PostgreSQL، شبکهٔ داخلی، volume ماندگار و عیب‌یابی چرخهٔ عمر آشنا شو.","Docker Compose: many services, one model","Build compose.yaml progressively, connect web to PostgreSQL, and investigate internal networking, persistent volumes, and lifecycle failures.","compose yaml services postgres service dns depends_on named volume up down build recreate"],
["09","09-env-secrets.html",1,18,150,"متغیرهای محیطی و secretها","یک image را میان محیط‌ها به‌کار ببر؛ تنظیمات را جدا کن و رمز را از image، Git و خروجی‌های ناامن دور نگه دار.","Environment variables and secrets","Reuse one image across environments, separate configuration, and keep credentials out of images, Git, and unsafe output.","env environment secret dotenv config interpolation env_file compose secrets rotation"],
...
["11","11-registry.html",1,18,130,"registry و push","از image محلی تا انتشار در registry؛ نام‌گذاری repository، tag و digest، ورود امن، push، pull و اثبات اجرای image بازیابی‌شده.","Registries, repositories, tags, and push","Publish a local image, read registry names and digests, authenticate safely, push and pull, then prove the retrieved image runs.","registry repository namespace tag digest docker hub ghcr login password-stdin push pull layers RepoDigests latest image distribution"],
["12","12-debug.html",1,18,160,"عیب‌یابی: exit code، OOM، پر شدن دیسک","از Exited 137 و خطای no space left تا مدرک، علت و اصلاح؛ بدون حدس یا پاک‌سازی پرخطر.","Troubleshooting: exit codes, OOM, and disk exhaustion","Trace exit status, OOM, and storage failures from evidence to a verified repair without destructive guesswork.","exit code 126 127 137 143 oom oomkilled memory disk full enospc df inode docker system df prune log driver"],
["13","13-security.html",1,18,170,"امنیت: non-root، read-only، محدودیت منابع","از کاربر غیرریشه و فایل‌سیستم فقط‌خواندنی تا سقف حافظه، CPU و پردازه؛ هر تغییر با آزمون و مدرک.","Container security: non-root, read-only filesystems, and resource limits","Reduce container authority, writable paths, and resource blast radius; verify each control with a working service.","security non-root user uid gid chown read-only readonly rootfs tmpfs volume memory mem_limit cpu cpus pids_limit docker.sock privileged capability Compose"],
["14","14-deploy.html",1,18,180,"استقرار روی VPS","از انتشار image نسخه‌دار تا reverse proxy، بررسی سلامت و rollback؛ با توضیح صادقانهٔ وقفهٔ ممکن.","Deploying Docker applications to a VPS","Ship versioned images, route through a reverse proxy, verify health, and roll back; understand single-container downtime.","vps deploy ssh registry caddy tls compose update rollback firewall"],
["15","15-cap1.html",1,0,180,"پروژهٔ ۱ — یک سرویس، یک ایمیج","یک API کوچک را خودت به image نسخه‌دار تبدیل کن، به registry بفرست و از نو pull و اجرا کن.","Project 1 — One service, one image","Build a versioned image for a small API, push it to a registry, then pull and run it from scratch.","project capstone node typescript dockerfile dockerignore multistage non-root healthcheck registry push pull limits",1],
["16","16-cap2.html",1,0,240,"پروژهٔ ۲ — اپ + پایگاه‌داده با Compose","دو سرویس، یک شبکهٔ داخلی، والیوم ماندگار، secret فایل‌محور و مدرک ماندگاری داده.","Project 2 — Web application + database with Docker Compose","Two services, an internal network, a persistent volume, a file-backed secret, and proof of data persistence.","capstone compose postgres volume secret healthcheck backup",2],
["17","17-cap3.html",1,0,480,"پروژهٔ ۳ — استقرار سه‌سرویسه روی سرور واقعی","Caddy، برنامه و PostgreSQL روی VPS؛ backup شبانه، آزمون restore، انتشار blue/green و rollback با شواهد.","Project 3 — Deploy a three-service application to a real server","Deploy Caddy, an application, and PostgreSQL to a VPS; prove nightly backup, restore, blue/green release, and rollback.","capstone production vps caddy postgres registry blue green zero downtime backup restore rollback",3]
]});
...
["03","03-server-block.html",1,18,70,"server block و virtual host","چند سایت روی یک IP و یک پورت.","Server blocks and virtual hosts","Many sites on one IP and one port.","server_name virtual host default_server sni"],
["04","04-location.html",1,18,80,"منطق location و اولویت‌ها","پیچیده‌ترین بخش Nginx، با جدول تصمیم.","Location matching and priority","The trickiest part of Nginx, with a decision table.","location regex prefix priority try_files"],
["05","05-static.html",1,18,70,"فایل استاتیک، کش مرورگر، فشرده‌سازی","gzip، brotli و هدرهای کش.","Static files, browser cache, compression","gzip, brotli and cache headers.","gzip brotli expires cache-control root alias"],
["06","06-proxy.html",1,18,80,"reverse proxy و هدرها","X-Forwarded-For و چیزهایی که اپ پشت پروکسی از دست می‌دهد.","Reverse proxy and headers","X-Forwarded-For and what your app loses behind a proxy.","proxy_pass proxy_set_header x-forwarded-for upstream"],
["07","07-lb.html",1,18,75,"load balancing و upstream","round-robin، least_conn، health و sticky session.","Load balancing and upstreams","round-robin, least_conn, health checks and sticky sessions.","upstream least_conn ip_hash keepalive"],
["08","08-tls.html",1,18,80,"HTTPS، Let's Encrypt، تمدید خودکار","گواهی واقعی در پنج دقیقه و تمدید بی‌دردسر.","HTTPS, Let's Encrypt, auto-renewal","A real certificate in five minutes and painless renewal.","ssl tls certbot letsencrypt hsts redirect"],
["09","09-ratelimit.html",1,18,70,"rate limiting و محافظت","limit_req، limit_conn و محافظت از فرم ورود.","Rate limiting and protection","limit_req, limit_conn and protecting a login form.","limit_req limit_conn burst nodelay"],
["10","10-logs.html",1,18,65,"لاگ و آنالیز","فرمت سفارشی، چرخش، و پیدا کردن کندترین مسیر.","Logs and analysis","Custom formats, rotation, and finding the slowest route.","access_log error_log log_format logrotate"],
["11","11-ws-upload.html",1,18,70,"WebSocket و آپلود حجیم","هدر Upgrade و client_max_body_size.","WebSockets and large uploads","The Upgrade header and client_max_body_size.","websocket upgrade client_max_body_size buffering"],
["12","12-tuning.html",1,18,75,"tuning: worker، buffer، timeout","عددهایی که واقعاً باید عوض شوند و آن‌هایی که نباید.","Tuning: workers, buffers, timeouts","The numbers worth changing and the ones that are not.","worker_processes worker_connections buffer timeout sendfile"],
["13","13-debug.html",1,18,75,"عیب‌یابی ۴۰۳/۵۰۲/۵۰۴","هر کد خطا یک علت مشخص دارد.","Debugging 403/502/504","Each status code points at a specific cause.","403 502 504 permission upstream timeout selinux"],
["14","14-cap1.html",1,5,60,"پروژهٔ ۱ — یک سایت استاتیک با HTTPS","سرو کردن فایل، فشرده‌سازی، کش و گواهی معتبر.","Project 1 — a static site with HTTPS","Serving files, compression, caching and a valid certificate.","capstone static",1],
["15","15-cap2.html",1,7,90,"پروژهٔ ۲ — پروکسی جلوی یک اپ","reverse proxy با هدرهای درست، WebSocket و آپلود حجیم.","Project 2 — proxying an app","A reverse proxy with correct headers, WebSockets and large uploads.","capstone proxy",2],
["16","16-cap3.html",1,9,150,"پروژهٔ ۳ — gateway کامل","سه سرویس پشت یک Nginx با SSL، کش، load balancing و rate limiting.","Project 3 — a complete gateway","Three services behind one Nginx with SSL, caching, load balancing and rate limits.","capstone gateway",3]
]});
...
C.push({
  ...
  ch:[
["01","01-shell.html",1,18,75,"شل و فایل‌سیستم","مسیر، ناوبری، و اینکه هر چیزی فایل است.","The shell and the filesystem","Paths, navigation, and everything-is-a-file.","bash ls cd path fhs"],
["02","02-perms.html",1,18,75,"کاربر، گروه، مجوز","chmod، chown و اینکه ۷۵۵ یعنی چه.","Users, groups, permissions","chmod, chown, and what 755 actually means.","chmod chown umask sudo group"],
["03","03-systemd.html",1,18,80,"پردازه و سرویس (systemd)","unit، سرویس خودت، و چرا بالا نمی‌آید.","Processes and services (systemd)","Units, your own service, and why it will not start.","systemd systemctl unit service journal"],
...
["02","02-model.html",1,18,85,"مدل داده: blob، tree، commit","Git یک پایگاه‌دادهٔ کلید-مقدار است. همه‌چیز از اینجا نتیجه می‌شود.","The data model: blobs, trees, commits","Git is a key-value store. Everything else follows from that.","blob tree commit sha object model plumbing"],
["03","03-basics.html",1,18,75,"سه ناحیه: working، staging، repository","چرا staging وجود دارد و چطور درست ازش استفاده کنیم.","Three areas: working, staging, repository","Why the staging area exists and how to use it properly.","add commit status staging index working tree"],
["04","04-history.html",1,18,80,"خواندن تاریخچه","log، show، diff — و پیدا کردن اینکه چه کسی چه چیزی را کِی عوض کرد.","Reading history","log, show, diff — and finding who changed what, when.","log diff show blame pickaxe"],
["05","05-branch.html",1,18,80,"شاخه: فقط یک اشاره‌گر","چرا ساختن شاخه در Git تقریباً رایگان است.","Branches: just a pointer","Why creating a branch in Git costs almost nothing.","branch checkout switch head pointer"],
["06","06-merge.html",1,18,85,"merge و حل تعارض","fast-forward، merge commit، و اینکه تعارض واقعاً یعنی چه.","Merging and resolving conflicts","Fast-forward, merge commits, and what a conflict actually is.","merge conflict fast-forward three-way"],
["07","07-rebase.html",1,18,90,"rebase","بازنویسی تاریخچه، و قانون طلایی‌اش.","Rebase","Rewriting history, and its golden rule.","rebase interactive squash fixup onto"],
["08","08-remote.html",1,18,80,"مخزن راه دور","fetch در برابر pull، push، و tracking branch.","Remotes","fetch versus pull, push, and tracking branches.","remote fetch pull push origin upstream tracking"],
["09","09-undo.html",1,18,90,"برگرداندن: reset، revert، restore","سه راه «برگرد عقب» که سه کار متفاوت می‌کنند.","Undoing: reset, revert, restore","Three ways to “go back” that do three different things.","reset revert restore hard soft mixed undo"],
["10","10-reflog.html",1,18,80,"reflog: تور نجات","ردّ حرکت refها را پیدا کن؛ و مرزهای واقعی بازیابی را بشناس.","Reflog: Git's local safety journal","Find ref movement clues—and understand the limits of recovery.","reflog recover lost commit dangling"],
["11","11-stash.html",1,18,80,"stash، cherry-pick، tag","کار نیمه‌تمام را امن کنار بگذار، hotfix مشخص را انتخابی بیاور و انتشار را با tag دقیق مشخص کن.","Stash, cherry-pick, and tags","Set unfinished work aside, apply a selected hotfix, and mark the release with an exact tag.","stash cherry-pick tag annotated"],
["12","12-bisect.html",1,18,80,"bisect: پیدا کردن commit خراب","با آزمون تکرارپذیر، مرز سالم و خراب را در تاریخچه پیدا کن و نتیجه را با patch ثابت کن.","Bisect: finding the commit that introduced a bug","Use a repeatable test to find the good/bad boundary and verify the culprit from its patch.","bisect regression debug automate"],
["13","13-workflow.html",1,18,85,"استراتژی شاخه‌بندی","عمر شاخه، انتشار، CI و نیازهای تیم را بسنج؛ trunk-based، GitHub Flow یا Git Flow را با هزینه‌ها و trade-offهایش انتخاب کن.","Branching strategies","Assess branch lifetime, releases, CI, and team needs; choose trunk-based, GitHub Flow, or Git Flow with their costs and trade-offs.","workflow gitflow trunk based feature branch"],
["14","14-collab.html",1,18,80,"کار تیمی: PR و بازبینی کد","commitهای معنادار، توضیح روشن PR، گفت‌وگوی سازندهٔ review، checkهای CI و انتخاب روش merge با دیدن تاریخچه.","Team collaboration: pull requests and code review","Meaningful commits, clear PR context, constructive review, CI checks, and choosing a merge method with history in mind.","pull request review conventional commit"],
["15","15-advanced.html",1,18,85,"ابزارهای پیشرفته","worktree، submodule، sparse checkout، hook و LFS.","Advanced Git tools: worktree, submodule, sparse-checkout, hooks, and LFS","worktree, submodules, sparse checkout, hooks and LFS.","worktree submodule hook lfs sparse"],
["16","16-recovery.html",1,18,85,"وقتی همه‌چیز خراب شد","از نشانه تا مدرک و تشخیص: نجات امن refها، commitها و دادهٔ Git، و شناخت مرز چیزی که هرگز ذخیره نشده است.","When everything goes wrong","Move from symptom to evidence and diagnosis: safely rescue refs, commits, and Git-stored data while recognizing what was never stored.","recovery detached head force push disaster"],
["17","17-cap1.html",1,0,100,"پروژهٔ ۱ — تاریخچهٔ تمیز","یک مخزن محلیِ آشفته را با staging دقیق و rebase تعاملی به تاریخچه‌ای خوانا تبدیل کن؛ محتوا را با آزمون و مقایسهٔ درخت ثابت کن.","Project 1 — Build a clean, readable history","Reshape a messy local repository with deliberate staging and interactive rebase; prove the result with tests and tree comparison.","capstone git history interactive rebase staging clean commits",1],
["18","18-cap2.html",1,0,160,"پروژهٔ ۲ — گردش کار تیمی","با bare remote و دو clone، دو feature هم‌زمان را با PR، review، حل تعارض و merge policy یکپارچه کن؛ همگامی آلیس و باب را با شواهد ثابت کن.","Project 2 — A real team workflow","Integrate two concurrent features through a bare remote and two clones; review, resolve a conflict, apply a merge policy, and prove both clones agree.","capstone team workflow bare remote clone fetch push pull request code review conflict merge policy",2],
["19","19-cap3.html",1,0,180,"پروژهٔ ۳ — نجات مخزن","cloneهای خراب را با reflog، rescue ref، بررسی objectها و نگه‌داشتن کار هم‌تیمی ترمیم کن؛ تفاوت دادهٔ بازیافتنی و فایلی را که Git ذخیره نکرده با مدرک گزارش بده.","Project 3 — Rescue a damaged repository without losing recoverable work","Preserve evidence, recover commits from reflogs and a teammate clone, repair the remote safely, and report what Git never stored.","capstone git rescue recovery reflog fsck unreachable force-push detached HEAD recoverable objects incident",3]
]});
